  ...
  actions: {
    async fetchAsistencias() {
      try {
        ...
        throw new Error("No se pudo cargar el reporte de asistencias. Intente nuevamente.");
      }
    },
    async procesarAsistencias(fechaInicio, fechaFin) {
        try {
          ...
          console.log("Fecha inicio (startDate):", new Date(startDate).toISOString());
          console.log("Fecha fin (endDate):", new Date(endDate).toISOString());
      
          // Filtrar asistencias dentro del rango
          const asistenciasEnRango = this.asistencias.filter(item => {
            ...
            if (!item.hora_entrada) {
              ...
              return false; // Si no existe hora_entrada, lo excluimos del filtro
            }
            ...
            console.log('hora entrada (como objeto Date):', horaEntrada.toISOString()); // Verificamos si la fecha es correcta
      
            // Verificar si la hora de entrada está dentro del rango
            const estaEnRango = horaEntrada >= startDate && horaEntrada <= endDate;
            console.log('¿Está en el rango?', estaEnRango); // Verificamos si el filtro pasa
      
            return estaEnRango;
          });
      
          console.log('Asistencias dentro del rango:', asistenciasEnRango); // Ver todas las asistencias dentro del rango
      
          return asistenciasEnRango.map((item) => {
            // Convertir hora_salida a formato Date y calcular las horas trabajadas
            const horaSalida = item.hora_salida ? new Date(item.hora_salida) : null;
      
            const horasTrabajadas = horaSalida
              ? ((horaSalida - new Date(item.hora_entrada)) / (1000 * 60 * 60)).toFixed(2)
